...
<form class="form-horizontal" action="{NV_BASE_ADMINURL}index.php?{NV_LANG_VARIABLE}={NV_LANG_DATA}&amp;{NV_NAME_VARIABLE}={MODULE_NAME}&amp;{NV_OP_VARIABLE}={OP}" method="post">
	...
	<div class="form-group">
		...
		<div class="col-sm-19 col-md-20">
			...
			<!-- BEGIN: radio_server_id -->
				<label><input class="form-control" type="radio" name="server_id" value="{OPTION.key}" {OPTION.checked}required="required" >{OPTION.title} &nbsp;</label> 
			<!-- END: radio_server_id -->
		</div>
	</div>
	<div class="form-group">
		<label class="col-sm-5 col-md-4 control-label"><strong>{LANG.tenant_id}</strong> <span class="red">(*)</span></label>
		<div class="col-sm-19 col-md-20">
			<select class="form-control" name="tenant_id">
				<option value=""> --- </option>
				<!-- BEGIN: select_tenant_id -->
				<option value="{OPTION.key}" {OPTION.selected}>{OPTION.title}</option>
				<!-- END: select_tenant_id -->
			</select>
		</div>
	</div>
	<div class="form-group">
